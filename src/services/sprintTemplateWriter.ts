...
export type SprintTask = {
    ...
    deadlinePlan: ScheduleValue;
    deadlineActual: ScheduleValue;
    aggregated: DurationValue;
    initials: string;
};
...
const initialColumnMap = new Map<string, { plan?: string; actual?: string }>();
...
export function mapIssueToSprintTask(issue: MappedIssue, resolveInitials?: (name: string) => string): SprintTask {
    ...
    const initials = resolveInitials ? resolveInitials(displayName) : computeInitials(displayName);
...
function computeScheduleSlip(timetracking: any, aggregatedSeconds?: number) {
...
function normalizeAggregatedSeconds(fields: any): number | undefined {
...
export function populateSprintScheduleWorksheet(sheet: ExcelJS.Worksheet, tasks: SprintTask[], comments: SprintComment[], resolveInitials?: (name: string) => string) {
...
function writeTasksSection(sheet: ExcelJS.Worksheet, tasks: SprintTask[]) {
...
function restoreTaskHeaders(sheet: ExcelJS.Worksheet) {
...
function writeCommentsSection(sheet: ExcelJS.Worksheet, comments: SprintComment[]) {
...
function writeAssigneeSection(sheet: ExcelJS.Worksheet, tasks: SprintTask[], totals: Map<string, { plan: number; actual: number }>) {
...
function formatTaskSummary(task: SprintTask) {
...
function formatComment(comment: SprintComment) {
...
function initialFromSegment(segment: string) {
    const alpha = segment.replace(/[^A-Za-z]/g, "");
    const source = alpha.length ? alpha : segment;
    return source.charAt(0) ?? "";
}

function resolveRole(displayName: string) {
...
function writeTimeCell(sheet: ExcelJS.Worksheet, address: string, value: DurationValue, zeroIfEmpty = false) {
...
function writeScheduleDateCell(sheet: ExcelJS.Worksheet, address: string, value: ScheduleValue) {
...
function writeScheduleSlipCell(sheet: ExcelJS.Worksheet, address: string, value: DurationValue) {
    const explicitText = value.text?.trim();
    if (explicitText) {
...
function hydrateInitialColumnMap(sheet: ExcelJS.Worksheet) {
    ...
    const resolvedInitials = (column: string) => resolveInitialsFromCell(sheet, `${column}${INITIAL_HEADER_ROW}`);
...
function resolveInitialsFromCell(sheet: ExcelJS.Worksheet, address: string) {
    const cell = sheet.getCell(address);
    const { value } = cell;
    ...
    if (typeof value === "object" && value !== null) {
        if ("result" in value && typeof (value as any).result === "string") {
            const initials = sanitizeInitialString((value as any).result);
            if (initials) {
                return initials;
            }
        }
        if ("formula" in value && typeof (value as any).formula === "string") {
            const referencedCell = sheet.getCell((value as any).formula);
...
function sanitizeInitialString(raw: string) {
...
function writeTaskInitialHours(sheet: ExcelJS.Worksheet, rowIndex: number, assigneeInitials: string, hours: number | undefined, isPlan: boolean) {
...
function clearInitialHourCells(sheet: ExcelJS.Worksheet, rowIndex: number, isPlan: boolean) {
    const columns = isPlan
        ? enumerateColumns(PLAN_INITIAL_START_COLUMN, PLAN_INITIAL_END_COLUMN)
        : enumerateColumns(ACTUAL_INITIAL_START_COLUMN, ACTUAL_INITIAL_END_COLUMN);
    columns.forEach((column) => clearCellValue(sheet, `${column}${rowIndex}`));
...
function enumerateColumns(startColumn: string, endColumn: string) {
...
function setCellValue(sheet: ExcelJS.Worksheet, address: string, value: string | number | null) {
    const cell = sheet.getCell(address);
    cell.value = value === undefined ? null : value;
}

function setNumericCell(sheet: ExcelJS.Worksheet, address: string, value: number, numberFormat?: string) {
    const cell = sheet.getCell(address);
    cell.value = value;
    if (numberFormat) {
...
function clearCellValue(sheet: ExcelJS.Worksheet, address: string) {
    const cell = sheet.getCell(address);
    cell.value = null;
}

function resolveSecondsValue(value: DurationValue) {
...
function formatScheduleDate(raw: string) {
...
function calculateAssigneeTotals(tasks: SprintTask[]) {
    const totals = new Map<string, { plan: number; actual: number }>();
    tasks.forEach(task => {
        const displayName = task.assigneeDisplayName?.trim();
        if (!displayName) {
            return;
...
function getHoursFromTimeCell(value: DurationValue) {
    const seconds = resolveSecondsValue(value);
    if (seconds !== undefined) {
...
function normalizeSummary(summary: unknown, fallbackKey: string | undefined) {
...
export function toDurationValue(seconds?: number | null, text?: string | null): DurationValue {
    return {
...
export function toScheduleValue(date?: string | null): ScheduleValue {
    return {
        date: typeof date === "string" ? date : undefined
    };
}

function writeTaskAssignmentMatrix(sheet: ExcelJS.Worksheet, tasks: SprintTask[]) {
...
function resolveInitialAddress(initials: string, isPlan: boolean, rowIndex: number): string | undefined {
    const mapping = initialColumnMap.get(initials);
